class Solution 
{
    public int carFleet(int target, int[] position, int[] speed) 
    {
        ...
        //Approach to the problem
            ...
            // This can be logically happened if we keep the positions in sorted order in ascending order so that the position of car whic is greater is the closest to the target
        ...
        int[][] positionWithSpeed = new int[n][2];
        for(int i=0;i<n;i++)
        {
        ...
        //SORT
        Arrays.sort(positionWithSpeed, Comparator.comparingInt(a->a[0]));
        ...
        for(int i=n-1;i>=0;i--)
        {
            ...
            double time = (double) (target-positionWithSpeed[i][0])/(double) positionWithSpeed[i][1];
            ...
            if(stack.size()>=2)
            {
                double currTargetSpeedPrevToClosest = stack.pop();
                double  closesttotargetSpeed  = stack.peek();
