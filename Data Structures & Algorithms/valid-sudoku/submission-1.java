...
/*
 * Context:
            	In a 9x9 Sudoku grid, there are 9 smaller 3x3 blocks (also called "subgrids" or "boxes"). These blocks are arranged as follows:
            	...
            	The idea is to calculate a unique index for each 3x3 block so that we can easily determine which block a given cell (i, j) belongs to.
            	...
            	This division by 3 works because the grid is divided into 3 groups of 3 rows and 3 columns. So, every 3 rows belong to a single group (block), and every 3 columns belong to a single group (block).

            	Multiplying by 3:
            	We multiply (i / 3) by 3 to scale it up, so it can be used to determine the "block row".
            	This is necessary because, for example, if i = 4 (the 5th row), then (i / 3) gives 1, indicating it's in the second block row (i.e., Block 4, Block 5, or Block 6).	
